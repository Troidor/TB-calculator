...
def range_to_coords_simple(range_text):
    ...
    if ":" in rt:
        ...
        # assume single-column ranges as in workbook
...
def py_ROUNDDOWN(x, n):
    ...
    # n is often negative in your sheet to round to tens
    if n >= 0:
        factor = 10 ** n
...
def py_RANK(value, ref_range, vm):
    ...
    # Excel's RANK gives 1 for highest; if duplicates returns first index+1
    return sorted_vals.index(v) + 1 if v in sorted_vals else None

def evaluate_all(vm, fm, max_passes=60):
    values = deepcopy(vm)
    # iterative passes to resolve dependencies
    for _ in range(max_passes):
        ...
        for cell, formula in fm.items():
            ...
            try:
                ...
                expr = re.sub(r"SUMIF\(([^,]+),([^,]+),([^)]+)\)", lambda m: "__SUMIF__('%s','%s','%s')"% (m.group(1).strip(), m.group(2).strip(), m.group(3).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"SUM\(([^)]+)\)", lambda m: "__SUM__('%s')" % (m.group(1).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"AVERAGE\(([^)]+)\)", lambda m: "__AVERAGE__('%s')" % (m.group(1).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"MAX\(([^)]+)\)", lambda m: "__MAX__('%s')" % (m.group(1).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"MIN\(([^)]+)\)", lambda m: "__MIN__('%s')" % (m.group(1).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"IF\(([^,]+),([^,]+),([^)]+)\)", lambda m: "__IF__(%s,%s,%s)" % (m.group(1).strip(), m.group(2).strip(), m.group(3).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"ROUNDDOWN\(([^,]+),([^)]+)\)", lambda m: "__ROUNDDOWN__(%s,%s)" % (m.group(1).strip(), m.group(2).strip()), expr, flags=re.IGNORECASE)
                expr = re.sub(r"RANK\(([^,]+),([^)]+)\)", lambda m: "__RANK__(%s,'%s')" % (m.group(1).strip(), m.group(2).strip()), expr, flags=re.IGNORECASE)
                # cell refs to __VAL__('A1')
                expr = re.sub(r"\b([A-Za-z]{1,3}\d{1,4})\b", lambda m: "__VAL__('%s')" % m.group(1), expr)
                # percent literals 12% -> (12/100)
                expr = re.sub(r"([0-9]*\.?[0-9]+)%", lambda m: "(%s/100)" % m.group(1), expr)
                # evaluation helpers
                ...
                # normalize ints that are floats
                if isinstance(result, float) and result.is_integer(): result = int(result)
                values[cell] = result
                changed = True
            except Exception:
                # ignore and continue; dependencies may be unresolved this pass
                pass
        if not changed:
            break
    return values
...
# UI: inputs
st.header('Inputs (B2 + percentages). Enter percentages as 100 = 100%')
inputs = {}
# B2 numeric
label_b2 = LABELS.get('A2', 'B2')
default_b2 = VALUES.get('B2', 0) or 0
try: default_b2 = float(default_b2)
except: default_b2 = 0.0
inputs['B2'] = st.number_input(f'{label_b2} (B2)', value=default_b2, format='%.6g')

# percentage inputs G8..G15 and G19..G26 (user enters 100 meaning 100%)
for coord in [f'G{r}' for r in range(8,16)] + [f'G{r}' for r in range(19,27)]:
    ...
    inputs[coord] = v / 100.0  # convert to decimal

if st.button('Calculate'):
    vm = deepcopy(VALUES)
    for k,v in inputs.items():
        vm[k] = v
    computed = evaluate_all(vm, FORMULAS, max_passes=80)
    st.header('Results — Output blocks')
